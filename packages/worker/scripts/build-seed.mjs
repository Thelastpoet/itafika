...
const statements = [
  ...
  insert("zones", ["id", "name", "type", "town", "county", "lat", "lng"], readCsv("zones.csv")),
  insert("modes", ["id", "label", "description", "source"], readCsv("modes.csv")),
  insert("providers", ["id", "name", "type", "reliability_score"], readCsv("providers.csv")),
  insert(
    "rates",
    ["provider_id", "origin_zone_id", "destination_zone_id", "base_cost_kes", "cost_per_kg_kes", "est_time", "max_weight_kg", "collection_type", "source"],
    readCsv("rates.csv"),
  ),
